...
* Any package with an `inst/staticexports/` directory. This includes the staticimports package itself, which contains many utility functions in [`inst/staticexports/`](https://github.com/wch/staticimports/tree/main/inst/staticexports).
...
The `pkg:staticimports` tells it to import from staticimports package. To import from a different package, use `pkg:mypackage`. It looks in a directory of the package named [`staticexports`](https://github.com/wch/staticimports/tree/main/inst/staticexports) to find the objects. To import from a local absolute or relative path, you can use something like `@staticimports ../r_utils/`.
...
```{r echo=FALSE}
import_objs(c("os_name", "%||%", "map", "walk"), outfile = stdout())
```
